        ...
        <ul class="list">
            ...
            @foreach($requirements['requirements'][$type] as $extention => $enabled)
                ...
                <li class="list__item {{ $enabled ? 'success' : 'error' }}">
                    ...
                    @if($extention == 'max_execution_time')
                        <br>
                        <small>Please Make sure your max execution time is greater
                            than {{$requirements['extensions_limit_array']['max_execution_time']}}</small>
                    @endif
                    @if($extention == 'upload_max_filesize')
                        <br>
                        <small>Please Make sure your mupload max filesize is greater
                            than {{$requirements['extensions_limit_array']['upload_max_filesize']}}MB</small>
                    @endif
                    @if($extention == 'post_max_size')
                        <br>
                        <small>Please Make sure your post max size is greater
                            than {{$requirements['extensions_limit_array']['post_max_size']}}MB</small>
                    @endif
